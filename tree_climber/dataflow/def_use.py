    ...
    if "ast_node" in attr:
        ...
        while q:
            ...
            if ast.nodes[n]["type"] == "identifier":
                _id = ast.nodes[n]["text"]
                if _id in solver.id2def.keys():
                    used_ids.add(_id)
            q.extend(ast.successors(n))
    return used_ids


def make_duc(ast, cfg, verbose=0):
    """
    ...
    solver = ReachingDefinitionSolver(ast, cfg, verbose=verbose)
    ...
    for n in cfg.nodes():
        ...
        if len(incoming_defs) > 0:
            ...
            used_ids = get_uses(ast, cfg, solver, use_node)
