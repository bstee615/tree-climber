...
class CppCFGVisitor(CFGVisitor):
    ...
    def _find_function_definitions_in_error(self, error_node: Node) -> List[Node]:
        """Recursively search for function definitions within an ERROR node."""
        function_defs = []
    ...
    def visit_ERROR(self, node: Node) -> CFGTraversalResult:
        """Handle ERROR nodes gracefully - treat them as statements"""
        # Create a statement node for the error to keep the CFG connected
    ...
    def visit_translation_unit(self, node: Node) -> CFGTraversalResult:
        ...
        if first_entry is None:
            ...
            error_msg = f"No valid function definitions found. Node types: {child_types}"
            
            # Create a minimal CFG with just entry and exit
            placeholder_entry = self.create_node(
                NodeType.ENTRY, source_text="ENTRY: Unparseable code"
            )
            placeholder_exit = self.create_node(
                NodeType.EXIT, source_text="EXIT: Unparseable code"
            )
