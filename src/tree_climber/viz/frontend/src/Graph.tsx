...
const DEF_USE_EDGES_KEY = 'tree_climber_show_def_use_edges';
...
const saveZoomInfo = (cy: cytoscape.Core) => {
  ...
  localStorage.setItem('tree_climber_zoom', JSON.stringify(zoomInfo));
}

function loadZoomAndPanInfo(cy: cytoscape.Core) {
  const zoomInfo = localStorage.getItem('tree_climber_zoom');
  if (zoomInfo) {
