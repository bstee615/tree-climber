...
def draw(cfg, dataflow_solution=None, ax=None):
    ...
    double_edges = [
        (u, v, attr.get("label", ""))
        for u, v, attr in cfg.edges(data=True)
        if u in cfg.successors(v) and v in cfg.successors(u)
    ]
    single_edges = [
        (u, v, attr.get("label", ""))
        for u, v, attr in cfg.edges(data=True)
        if (u, v, attr.get("label", "")) not in double_edges
    ]
    nx.draw_networkx_edges(
        cfg, pos=pos, edgelist=[(u, v) for u, v, label in single_edges], ax=ax
    )
    nx.draw_networkx_edge_labels(
        cfg, pos=pos, edge_labels={(u, v): label for u, v, label in single_edges}
    )
    nx.draw_networkx_edges(
        cfg,
        pos=pos,
        edgelist=[(u, v) for u, v, label in double_edges],
        connectionstyle=f"arc3,rad=0.1",
    )
    nx.draw_networkx_edge_labels(
        cfg, pos=pos, edge_labels={(u, v): label for u, v, label in double_edges}
    )  # , connectionstyle=f"arc3,rad=0.1")
    ...
    if dataflow_solution is not None:
        nx.draw_networkx_labels(
            ...
            font_color="r",
            labels={
                n: "\n\n\n{" + ", ".join(dataflow_solution[n]) + "}"
                if dataflow_solution[n]
                else ""
                for n in cfg.nodes()
            },
            ax=ax,
...
def get_node_by_code(cfg, code, get="first"):
    matches = (
        n for n, attr in cfg.nodes(data=True) if code == attr.get("text", "<NO CODE>")
    )
    if get == "first":
        return next(matches)
    elif get == "all":
...
def get_node_by_label(cfg, label):
    return next(
        ...
        if label == attr.get("label", "<NO LABEL>")
    )
