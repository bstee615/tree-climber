...
def debug_simple_if():
    ...
    samples_dir = os.path.join(
        os.path.dirname(__file__), "..", "java_cfg_samples", "basic_constructs.java"
    )
    ...
    try:
        ...
        if condition_nodes:
            ...
            print(
                f"Found condition node {condition_node.id}: '{condition_node.source_text}'"
            )
            ...
            for succ_id in condition_node.successors:
                ...
                print(
                    f"  -> {succ_id} ({succ_node.node_type.name}): '{succ_node.source_text}' [label: '{edge_label}']"
                )
        else:
