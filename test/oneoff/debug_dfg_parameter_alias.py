...
def analyze_dfg(language, code):
    ...
    for var_name, chains in def_use_result.chains.items():
        ...
        for chain in chains:
            ...
            print(f"    Definition at node {chain.definition}: '{def_node.source_text.strip()}'")
            for use_id in chain.uses:
                use_node = cfg.nodes[use_id]
                print(f"      -> Used at node {use_id}: '{use_node.source_text.strip()}'")
        ...
        for chain in chains:
            ...
            for def_id in chain.definitions:
                if def_id in cfg.nodes:
                    ...
                    print(f"      <- Defined at node {def_id}: '{def_node.source_text.strip()}'")
                else:
