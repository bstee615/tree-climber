...
from tree_climber.dataflow.analyses.reaching_definitions import ReachingDefinitionsProblem
...
class DFGTestHelper:
    ...
    def assert_def_use_chain(self, def_use_result: any, variable: str, 
                           def_source: str, use_source: str, msg: str = ""):
        ...
        for chain in chains:
            ...
            def_node = self.current_cfg.nodes.get(chain.definition) if self.current_cfg else None
            ...
            if def_source in def_text:
                ...
                for use_id in chain.uses:
                    use_node = self.current_cfg.nodes.get(use_id) if self.current_cfg else None
    ...
    def assert_use_def_chain(self, use_def_result: any, variable: str,
                           use_source: str, def_source: str, msg: str = ""):
        ...
        for chain in chains:
            # Get actual source text from CFG nodes
            use_node = self.current_cfg.nodes.get(chain.use) if self.current_cfg else None
            use_text = use_node.source_text.strip() if use_node else ""
            
            if use_source in use_text:
                # Check if any definition matches def_source
                for def_id in chain.definitions:
                    def_node = self.current_cfg.nodes.get(def_id) if self.current_cfg else None
                    def_text = def_node.source_text.strip() if def_node else ""
                    if def_source in def_text:
                        matching_chain = chain
                        break
    ...
    def assert_parameter_alias(self, def_use_result: any, parameter: str, 
                             argument_def: str, parameter_use: str, msg: str = ""):
    ...
    def assert_variable_count(self, def_use_result: any, expected_count: int, msg: str = ""):
    ...
    def debug_dfg_chains(self, cfg: CFG, def_use_result: any, use_def_result: any, title: str = "DFG Debug"):
        ...
        for var_name, chains in def_use_result.chains.items():
            ...
            for chain in chains:
                ...
                def_text = def_node.source_text.strip() if def_node else f"node_{chain.definition}"
                print(f"    Definition at node {chain.definition}: '{def_text}'")
                for use_id in chain.uses:
                    use_node = cfg.nodes.get(use_id)
                    use_text = use_node.source_text.strip() if use_node else f"node_{use_id}"
        ...
        for var_name, chains in use_def_result.chains.items():
            ...
            for chain in chains:
                ...
                for def_id in chain.definitions:
                    ...
                    def_text = def_node.source_text.strip() if def_node else f"node_{def_id}"
    ...
    def _format_def_use_chains(self, chains: list) -> str:
        ...
        for chain in chains:
            ...
            formatted_chains.append(f"def '{def_text}' -> uses [{', '.join(use_texts)}]")
    ...
    def _format_use_def_chains(self, chains: list) -> str:
        ...
        for chain in chains:
            ...
            formatted_chains.append(f"use '{use_text}' <- defs [{', '.join(def_texts)}]")
...
class TestBasicDefUse:
    ...
    def test_simple_assignment(self, helper):
        ...
        helper.assert_variable_count(def_use_result, 2, "Should track x and y variables")
        
        # x should be defined and used in y assignment
        helper.assert_def_use_chain(def_use_result, "x", "x = 5", "y = x + 1", "x definition should reach its use")
        
        # y should be defined and used in return
        helper.assert_def_use_chain(def_use_result, "y", "y = x + 1", "return y", "y definition should reach return")
    ...
    def test_variable_redefinition(self, helper):
        ...
        assert len(x_chains) == 2, f"Expected 2 def-use chains for x, got {len(x_chains)}"
        
        # First definition (x = 5) should reach y assignment
        helper.assert_def_use_chain(def_use_result, "x", "x = 5", "y = x", "First x definition should reach y")
        
        # Second definition (x = 10) should reach z assignment  
        helper.assert_def_use_chain(def_use_result, "x", "x = 10", "z = x", "Second x definition should reach z")
    ...
    def test_conditional_definitions(self, helper):
        ...
        assert y_assignment_chain is not None, "Should find use-def chain for x in y assignment"
        # Should have definitions from both conditional branches
        assert len(y_assignment_chain.definitions) >= 1, "x use should have at least one reaching definition"
...
class TestInterproceduralAnalysis:
    ...
    def test_simple_parameter_alias(self, helper):
        ...
        helper.assert_parameter_alias(def_use_result, "a", "x = 5", "b = a + 1", 
    ...
    def test_multiple_parameters(self, helper):
        ...
        helper.assert_parameter_alias(def_use_result, "a", "x = 5", "return a + b", 
                                    "Parameter 'a' should alias to argument 'x'")
        helper.assert_parameter_alias(def_use_result, "b", "y = 3", "return a + b", 
    ...
    def test_nested_function_calls(self, helper):
        ...
        helper.assert_parameter_alias(def_use_result, "y", "a = 5", "leaf(y)", 
                                    "Parameter 'y' should alias to argument 'a'")
        helper.assert_parameter_alias(def_use_result, "x", "a = 5", "return x * 2", 
...
class TestComplexDataflow:
    ...
    def test_loop_with_function_calls(self, helper):
        ...
        assert len(val_chains) >= 1, f"Expected at least 1 def-use chain for val, got {len(val_chains)}"
    ...
    def test_parameter_modification(self, helper):
        ...
        assert len(p_chains) >= 2, f"Expected at least 2 def-use chains for p, got {len(p_chains)}"
